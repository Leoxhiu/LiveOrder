...
PurchaseReportAction::PurchaseReportAction(string date, int accountType) {
	Initialization(date, accountType);
}

void PurchaseReportAction::Initialization(string date, int accountType) {
	cout << "============================" << endl;
	cout << "   Purchase Report Actions  " << endl;
	cout << "============================" << endl;

	OrderDA orderDA;
	orderDA.filterOrderbyDate(date, orderDA.getOrderData());
	...
	case 1: {
		Screen::clearScreen();
		filterReport(date, accountType);
		break;
	}
	case 2: {
		Screen::clearScreen();
		sortReport(date, accountType);
		break;
	}
	case 3: {
		Screen::clearScreen();
...
void PurchaseReportAction::filterReport(string date, int accountType) {
	cout << "============================" << endl;
	...
	case 1: {
		...
		do {
			...
			if (ItemID < 1 || ItemID > 10) {
				...
				orderDA.filterOrderbyDate(date, tempOrder);
				keepLaunch("Continue?", date, accountType);
				break;
			}
...
void PurchaseReportAction::sortReport(string date, int accountType) {
	cout << "============================" << endl;
	...
	case 1: {
		...
		do {
			...
			if (sortBy != 1 && sortBy != 2) {
				...
				orderDA.filterOrderbyDate(date, orderData);
				keepLaunch("Continue?", date, accountType);
				break;
			}
			else if (sortBy == 2) {
...
void PurchaseReportAction::keepLaunch(string message, string date, int accountType) {
	int option;
	...
	default: {
		...
		this_thread::sleep_for(std::chrono::seconds(3));
		Screen::clearScreen();
		keepLaunch(message, date, accountType);
	}
	}
}